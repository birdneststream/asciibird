...
/** Clear all blocks in a grid selection area. Mutates layerBlocks in place. */
...
function updateSelection(
  deps: SelectionTransformDeps,
  x: number, y: number, w: number, h: number,
  layerBlocks: Block[][],
): void {
  deps.selecting.value = {
    startX: x * deps.blockWidthComp.value,
    startY: y * deps.blockHeightComp.value,
    endX: (x + w) * deps.blockWidthComp.value,
    endY: (y + h) * deps.blockHeightComp.value,
    canSelect: false,
  };
  const newRect = selectionToGridRect(
    deps.selecting.value, deps.blockWidthComp.value,
    deps.blockHeightComp.value,
...
/** Apply a rotation or flip transform to the current selection. */
async function applyTransform(
  ...
  updateSelection(deps, rect.x, rect.y, clampedW, clampedH, layerBlocks);

  await commitDiff(deps, oldDiffs, newDiffs, layerBlocks);
  return true;
}

/** Nudge the current selection by (dx, dy) grid cells. */
async function applyNudge(
  ...
  if (clampedW > 0 && clampedH > 0) {
    updateSelection(deps, clampedX, clampedY, clampedW, clampedH, layerBlocks);
  }
...
export function useSelectionTransform(deps: SelectionTransformDeps) {
  return {
    applyTransform: (type: TransformType) => applyTransform(deps, type),
    applyNudge: (dx: number, dy: number) => applyNudge(deps, dx, dy),
    selectionToGridRect,
